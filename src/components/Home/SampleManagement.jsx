...
const SampleDirectory = ({ samples, pagination, loading, error, onRequest }) => {
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-industrial-200 bg-white shadow-sm flex flex-col">
      ...
      {loading ? <TableSkeleton /> : error ? (
        ...
        <div className="px-6 py-12 text-center">
          <p className="text-sm font-bold text-industrial-900">{query ? 'No samples found' : 'No samples available'}</p>
          <p className="mt-1 text-xs text-industrial-500">{query ? 'Try a different search term.' : 'There are currently no samples configured.'}</p>
        </div>
      )}
    </section>
  );
...
const SampleManagement = ({ samples, pagination, loading, error, onRequestSamples, onCreateSample }) => {
  const handleCreateSample = async (payload) => {
    try {
...
const sampleShape = PropTypes.shape({ sampleId: PropTypes.string.isRequired, userId: PropTypes.string, userName: PropTypes.string, createdDate: PropTypes.string });
...
SampleDirectory.propTypes = { samples: PropTypes.arrayOf(sampleShape).isRequired, pagination: paginationShape.isRequired, loading: PropTypes.bool.isRequired, error: PropTypes.bool.isRequired, onRequest: PropTypes.func.isRequired };
SampleManagement.propTypes = { samples: PropTypes.arrayOf(sampleShape).isRequired, pagination: paginationShape.isRequired, loading: PropTypes.bool.isRequired, error: PropTypes.bool.isRequired, onRequestSamples: PropTypes.func.isRequired, onCreateSample: PropTypes.func.isRequired };
